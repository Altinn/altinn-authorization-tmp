...
- **Pure-logic helpers in `Altinn.AccessMgmt.Persistence.Core` and
  `Altinn.AccessMgmt.Core`.** Builders, validation rules, mappers,
  utilities with computational behavior. Skip the EF / repository
  pass-through layer (covered by live-DB tests below).
...
- **`Altinn.Authorization.Host.Pipeline`** — ~1.4k LOC, currently
  only the Step 4 smoke test. The test project is scaffolded;
  populate it with real tests for builders, pipeline services, and
  segment / sink / source code.
