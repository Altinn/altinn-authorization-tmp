...
|---|---|---|---|
| `AccessMgmt.Tests` | Controller/integration tests; `LegacyApiFixture` for the tail still backed by the Dapper repositories on the legacy schemas, `ApiFixture` for the rest, `EfDatabaseFixture` for no-host EF service/repository tests. | `ApiFixture`, `LegacyApiFixture`, `EfDatabaseFixture` | ✅ |
