...
public class ResourceOwnerAuthorizedPartiesController : ControllerBase
{
    ...
    public async Task<ActionResult<List<AuthorizedPartyExternal>>> GetAuthorizedPartiesAsServiceOwner(BaseAttributeExternal subject, bool includeAltinn2 = false, bool includeAltinn3 = true, CancellationToken cancellationToken = default)
    {
        try
        {
            BaseAttribute subjectAttribute = _mapper.Map<BaseAttribute>(subject);
            List<AuthorizedParty> authorizedParties = await _authorizedPartiesService.GetAuthorizedParties(subjectAttribute, includeAltinn2, includeAltinn3, cancellationToken);
