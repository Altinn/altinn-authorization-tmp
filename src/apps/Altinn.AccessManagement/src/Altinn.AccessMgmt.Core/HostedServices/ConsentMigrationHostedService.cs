...
public partial class ConsentMigrationHostedService : BackgroundService
{
  ...
  protected override async Task ExecuteAsync(CancellationToken stoppingToken)
  {
    ...
    while (!stoppingToken.IsCancellationRequested)
    {
      try
      {
        ...
        if (!isEnabled)
        {
          _logger.LogDebug("Consent migration is disabled by feature flag");
        ...
        if (processedCount == 0)
        {
          _logger.LogInformation("No consents available for migration. Waiting {Delay}ms", _settings.EmptyFeedDelayMs);
  ...
  static partial class Log
  {
    ...
    [LoggerMessage(EventId = 2, Level = LogLevel.Warning, Message = "Consent migration end date {endDate} reached. Stopping service.")]
