...
public class RightholderRemovedNotificationHandler(AppDbContext db, IFeatureManager featureManager) : IOutboxHandler
{
    ...
    {
        if (await featureManager.IsDisabledAsync(AccessMgmtFeatureFlags.AccessMgmtCoreOutboxRightholderNotifyRemoved, cancellationToken))
        {
            db.OutboxMessageLogs.Add(message, $"Feature flag '{AccessMgmtFeatureFlags.AccessMgmtCoreOutboxRightholderNotifyRemoved}' is disabled.");
