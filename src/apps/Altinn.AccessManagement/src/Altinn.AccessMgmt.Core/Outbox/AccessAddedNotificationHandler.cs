...
public class AccessAddedNotificationHandler(
    AppDbContext db,
    ...
    IEntityService entityService) : IOutboxHandler
{
    ...
    private async Task<(Entity Recipient, Entity Requester, IEnumerable<Resource> Resources, IEnumerable<Package> Packages, string IdempotencyId)> UnwrapMessage(OutboxMessage message, CancellationToken cancellationToken)
    {
        ...
        async Task<List<Resource>> GetResources(AccessAddedNotificationMessage content, CancellationToken cancellationToken)
        {
        ...
        async Task<List<Package>> GetPackages(AccessAddedNotificationMessage content, CancellationToken cancellationToken)
        {
    ...
    private static string MailContent(Entity from, Entity to, IEnumerable<Resource> resources, IEnumerable<Package> packages)
    {
        ...
        return @$"
            ...
            {access}
            <p>
                Du finner mer informasjon ved å logge deg inn på Altinn sine nettsider, velge riktig bruker og gå til Tilgangsstyring i menyen.
            </p>
            <p>
