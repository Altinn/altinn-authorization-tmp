...
public class ClientDelegationService(AppDbContext db, IOptions<CoreAppsettings> appsettings) : IClientDelegationService
{
    ...
    public async Task<Result<List<MyClientDto>>> GetMyClients(Guid partyUuid, List<Guid> providerUuids, CancellationToken cancellationToken = default)
    {
        ...
        var baseQuery = db.Assignments
            ...
            .SelectMany(x => x.Packages.DefaultIfEmpty(), (x, dp) => new { x.Client, x.Provider, x.Role, x.Delegation, PackageId = dp != null ? dp.PackageId : Guid.Empty })
            .GroupJoin(
                db.DelegationResources,
                x => x.Delegation.Id,
                dr => dr.DelegationId,
                (x, dr) => new { x.Client, x.Provider, x.Role, x.PackageId, Resources = dr }
            )
            .SelectMany(x => x.Resources.DefaultIfEmpty(), (x, dr) => new { x.Client, x.Provider, x.Role, x.PackageId, ResourceId = dr != null ? dr.ResourceId : Guid.Empty });
