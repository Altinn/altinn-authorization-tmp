...
public interface IDelegationMetadataRepository
{
    ...
    /// <param name="toRuntimeDelegated">All To parties with runtime delegated permissions to use for lookup</param>
    /// <param name="cancellationToken">The <see cref="CancellationToken"/></param>
    /// <returns>The complete InstanceDelegationChange record stored in the database</returns>
    Task<IEnumerable<InstanceDelegationChange>> GetActiveInstanceDelegations(List<string> resourceIds, Guid from, List<Guid> to, List<Guid> toRuntimeDelegated = null, CancellationToken cancellationToken = default);
