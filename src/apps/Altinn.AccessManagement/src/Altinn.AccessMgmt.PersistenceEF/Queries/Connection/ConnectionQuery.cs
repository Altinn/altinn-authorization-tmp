...
public class ConnectionQuery(AppDbContext db)
{
    ...
    public async Task<List<ConnectionQueryExtendedRecord>> GetConnectionsAsync(ConnectionQueryFilter filter, ConnectionQueryDirection direction, CancellationToken ct = default)
    {
        try
        {
            bool useNewExprQueries = true;

            var baseQuery = direction == ConnectionQueryDirection.FromOthers 
                ? useNewExprQueries ? await BuildBaseQueryFromOthersNew(db, filter) : BuildBaseQueryFromOthers(db, filter)
    ...
    {
        var baseQuery = direction == ConnectionQueryDirection.FromOthers
                ? useNew ? await BuildBaseQueryFromOthersNew(db, filter) : BuildBaseQueryFromOthers(db, filter)
    ...
    private async Task<IQueryable<ConnectionQueryBaseRecord>> BuildBaseQueryFromOthersNew(AppDbContext db, ConnectionQueryFilter filter)
    {
        ...
        */
        ...
        var includeFilter = filter.ToIds.ToList();
        ...
        includeFilter.AddRange(keyRoleIds);

        var includeSet = new HashSet<Guid>(includeFilter);
