...
namespace Altinn.AccessManagement.Api.Enterprise.Controllers
{
    ...
    public class ConsentController(IConsent consentService, IPDP Pdp) : ControllerBase
    {
        ...
        public async Task<ActionResult> CreateRequest([FromBody] ConsentRequestDto consentRequest, CancellationToken cancellationToken = default)
        {
            ...
            Result<ConsentRequestDetailsWrapper> consentRequestStatus = await _consentService.CreateRequest(consentRequestInternal, consentPartyUrn, cancellationToken, false);
