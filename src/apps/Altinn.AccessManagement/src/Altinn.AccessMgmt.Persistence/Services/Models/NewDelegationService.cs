    ...
    )
{
    ...
    public async Task CreateClientDelegation(NewDelegationRequest request, Guid userId)
    {
        // Find user : Fredrik
        ...
        // Find Agent
        var agent = await GetOrCreateEntity(request.AgentId, request.AgentId.ToString(), request.AgentId.ToString(), "System", "System") ?? throw new Exception(string.Format("Could not find or create '{0}'", request.AgentId));
