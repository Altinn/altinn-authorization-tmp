...
/// </summary>
public class RelationPermissionRepository : ExtendedRepository<Relation, ExtRelation>, IRelationPermissionRepository
{
    /// <inheritdoc/>
    public RelationPermissionRepository(DbDefinitionRegistry dbDefinitionRegistry, IDbExecutor executor) : base(dbDefinitionRegistry, executor)
    {
    }

    /// <inheritdoc/>
    public async Task<IEnumerable<PackageDelegationCheck>> GetAssignableAccessPackages(Guid fromId, Guid toId, IEnumerable<Guid> packageIds, CancellationToken cancellationToken = default)
    {
        return await executor.ExecuteQuery(
            AssignableAccessPackagesQuery(),
            new List<GenericParameter>
            {
                new GenericParameter("fromId", fromId),
                new GenericParameter("toId", toId),
                new GenericParameter("packageIds", packageIds)
            },
            GetPackageDelegationCheck,
            callerName: nameof(GetAssignableAccessPackages),
    ...
    private static async ValueTask<PackageDelegationCheck> GetPackageDelegationCheck(NpgsqlDataReader reader)
    {
        return new PackageDelegationCheck
        {
            ...
            Reason = new PackageDelegationCheckReason
            {
