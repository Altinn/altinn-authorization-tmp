...
public class FuzzySearchTest
{
    ...
    [Fact]
    public void PerformFuzzySearch_MultiWordTerm_FindsMultiWordValue()
    {
        var data = new List<Item> { new("Apple Juice", "Drink") };
        var results = FuzzySearch.PerformFuzzySearch(data, "Apple Juice", NameOnlyBuilder());
        Assert.NotEmpty(results);
    }
    ...
    [Fact]
    public void PerformFuzzySearch_MultipleInputs_ReturnsBothMatches()
    {
        var data = new List<Item>
        {
            new("Applet", "Software"),
        ...
        // Both "Apple" and "Applet" should match the search term "Apple"
        Assert.True(results.Count >= 1);
        Assert.Contains(results, r => r.Object.Name == "Apple");
    }
