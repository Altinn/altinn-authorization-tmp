...
public class BrregApiWrapper
{
    ...
    public async Task<Unit?> GetUnit(string orgNo)
    {
        ...
        {
            using var client = new HttpClient();
            var res = await client.GetFromJsonAsync<Unit>($"https://data.brreg.no/enhetsregisteret/api/enheter/{orgNo}");
            if (res == null)
            {
                return null;
            }
    ...
    {
        using var client = new HttpClient();
        var res = await client.GetFromJsonAsync<Unit>($"https://data.brreg.no/enhetsregisteret/api/underenheter/{orgNo}");
        return res;
    }
