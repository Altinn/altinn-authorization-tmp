...
public class Engine
{
    ...
    {
        var res = await resourceService.Get(t => t.RefId, rawResource.Identifier);
    ...
    private async Task<ResourceType> GetOrCreateResourceType(string name)
    {
        ...
        var res = await resourceTypeService.Get(t => t.Name, name);
        if (res == null || res.Count() == 0)
        {
    ...
    private async Task<Provider> GetOrCreateProvider(string name, string refId)
    {
        ...
        var res = await providerService.Get(t => t.Name, name); //// RefId?
