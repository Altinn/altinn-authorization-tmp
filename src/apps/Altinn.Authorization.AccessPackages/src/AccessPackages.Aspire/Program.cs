...
var web = builder.AddProject<Altinn_Authorization_FFB>("web").WaitForCompletion(cli);//.WaitFor(api);

builder.AddProject<Projects.AccessPackages_MetaWeb>("accesspackages-metaweb").WaitForCompletion(cli);
//var api = builder.AddProject<Altinn_Authorization_AccessPackages>("api").WaitForCompletion(cli); // .WaitFor(brreg).WaitFor(resreg);
