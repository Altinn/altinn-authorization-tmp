...
public class Mockups
{
    ...
    private readonly IDelegationPackageResourceService delegationPackageResourceService;
    private readonly IDelegationResourceService delegationResourceService;
    ...
    /// <param name="delegationPackageResourceService">IDelegationPackageResourceService</param>
    ...
    public Mockups(
        ...
        IDelegationPackageResourceService delegationPackageResourceService,
        IDelegationResourceService delegationResourceService
        )
    {
        this.logger = logger;
        ...
        this.delegationPackageResourceService = delegationPackageResourceService;
        this.delegationResourceService=delegationResourceService;
    }
    #endregion
    ...
    private List<DelegationPackageResource> DelegationPackageResources { get; set; }

    private List<DelegationResource> DelegationResources { get; set; }
    ...
    public async Task LoadCache(bool ignoreEntity = false, bool ignoreAssignment = false)
    {
        ...
        DelegationPackageResources = [.. await delegationPackageResourceService.Get()];
        DelegationResources = [.. await delegationResourceService.Get()];
    ...
    private async Task<DelegationPackageResource> GetOrCreateDelegationPackageResource(DelegationPackageResource obj)
    {
        var res = DelegationPackageResources.FirstOrDefault(t => t.DelegationId == obj.DelegationId && t.PackageResourceId == obj.PackageResourceId) ?? null;
        if (res == null)
        {
            await delegationPackageResourceService.Create(obj);
            DelegationPackageResources.Add(obj);
            return obj;
        }
        else
        {
    ...
    private async Task<DelegationResource> GetOrCreateDelegationResource(DelegationResource obj)
    {
        var res = DelegationResources.FirstOrDefault(t => t.DelegationId == obj.DelegationId && t.RoleResourceId == obj.RoleResourceId && t.AssignmentResourceId == obj.AssignmentResourceId) ?? null;
        if (res == null)
        {
            await delegationResourceService.Create(obj);
            DelegationResources.Add(obj);
            return obj;
        }
        else
        {
    ...
    public async Task KlientDelegeringMock()
    {
        ...
        //await GetOrCreateDelegationPackageResource(new DelegationPackageResource() { Id = Guid.NewGuid(), DelegationId = regnDelegation.Id, PackageId = regnPakke1.Id });
        //await GetOrCreateDelegationPackageResource(new DelegationPackageResource() { Id = Guid.NewGuid(), DelegationId = regnDelegation.Id, PackageId = regnPakke3.Id });

        //var reviDelegation = await GetOrCreateDelegation(new Delegation() { Id = Guid.NewGuid(), AssignmentId = ass03_2.Id });
        //await GetOrCreateDelegationPackageResource(new DelegationPackageResource() { Id = Guid.NewGuid(), DelegationId = reviDelegation.Id, PackageId = regnPakke2.Id });
    ...
    private async Task GenerateOrg(int id, string industry, AssignmentSpec assignmentSpec)
    {
        var comp = await GetOrCreateEntity(GenerateOrgEntity(GenerateFakeGuid("entity", 0, id), industry), true);

        for (int i = 0; i < assignmentSpec.Roles.Values.Max(); i++)
        {
            var person = await GetOrCreateEntity(GeneratePersonEntity(GenerateFakeGuid("entity", id, i)), true);
            foreach (var roleSpec in assignmentSpec.Roles)
            {
                if (roleSpec.Value == i)
                {
                    var role = GetRole(roleSpec.Key);
                    await GetOrCreateAssignment(new Assignment() { Id = Guid.NewGuid(), FromId = comp.Id, ToId = person.Id, RoleId = role.Id, IsDelegable = true });
                }
            }
        }
    }
    private int GetRangeValue(OldRange range)
    {
        return Random.Next(range.Min, range.Max);
    }

    private (Dictionary<string, int> a, List<(int no, List<string> roles)> b) CommonSpec(int ansatte, int medl, bool dagligLeder = true, bool styreLeder = true)
    {
        var randomAssignments = new Dictionary<string, int>();
        var distinctAssignments = new List<(int Number, List<string> Roles)>();

        randomAssignments.Add("ANSATT", ansatte);
        randomAssignments.Add("MEDL", medl);

        if (dagligLeder && styreLeder)
        {
            distinctAssignments.Add(new(1, ["DAGL", "LEDE"]));
        }
        else
        {
            if (dagligLeder)
            {
                distinctAssignments.Add(new(1, ["DAGL"]));
            }
            if (styreLeder)
            {
                distinctAssignments.Add(new(1, ["LEDE"]));
            }
        }

        return (randomAssignments, distinctAssignments);
    }

    private AssignmentSpec GenerateAssignmentSpec(bool regnIsOrg = true, bool reviIsOrg = true)
    {
    ...
    public async Task BasicMock()
    {
        ...
        var ddd = new List<(string industry, int antall, int ansatte)>()
        {
        ...
        foreach (var specOrg in ddd)
        {
            orgs.Add(specOrg.industry, new List<Entity>());
            int medlCount = (int)Math.Round((10.0 / 100) * specOrg.ansatte, MidpointRounding.AwayFromZero);
            ...
            for (int j = 0; j < specOrg.antall; j++)
            {
                string orgKey = $"entity:org:{i}:{j}";
                var comp = await GetOrCreateEntity(GenerateOrgEntity(GenerateFakeGuid(orgKey), specOrg.industry), true);
                orgs[specOrg.industry].Add(comp);

                int employeeCount = specOrg.ansatte;
                ...
                /*PEOPLE + ASSIGNMENTS*/
                for (int k = 0; k < specOrg.ansatte; k++)
                {
        ...
        foreach(var assignment in assignments.Where(t=>t.RoleId == regn.Id))
        {
            ...
            if (fromSize >= toSize)
            {
                ...
                for(int ra = 0; ra < 1 + diff; ra++)
                {
                    ...
                    var packagesToDelegate = new List<Guid>();
                    var resourcesToDelegate = new List<Guid>();
                    ...
                    foreach(var rolePack in rolePacks)
                    {
                        ...
                        {
                            GetOrCreateDelegationPackageResource(new DelegationPackageResource() { Id = GenerateFakeGuid(""), DelegationId = delegation.Id, PackageResourceId = rolePack.Id });
                            packagesToDelegate.Add(rolePack.PackageId);
                            continue;
                        }
                    ...
                    foreach (var assPack in assPacks)
                    {
                        ...
                        if (diff % 1 == 1)
                        {
                            packagesToDelegate.Add(assPack.PackageId);
                            continue;
                        }
                    }
                    ...
                    foreach (var roleRes in roleResources)
                    {
                        ...
                        {
                            GetOrCreateDelegationPackageResource(new DelegationResource() { Id = GenerateFakeGuid(""), DelegationId = delegation.Id,  })
                    ...
                    foreach(var pack in packagesToDelegate)
                    {
        ...
        */
        ...
        En metode som gir meg en Random Entity av gitt bransje
        ...
        */
        ...
        //await GetOrCreateDelegationPackageResource(new DelegationPackageResource()
        //{
        //    Id = Guid.NewGuid(),
    ...
    {
        var comp = GenerateCompanyName(industry);
        var type = GetEntityType("Organisasjon");
        var variant = GetEntityVariant("Organisasjon", "AS");
        return new Entity { Id = id, Name = comp, RefId = GenerateOrganizationNumber(), TypeId = type.Id, VariantId = variant.Id };
    }

    private Entity GeneratePersonEntity(Guid id, string overrideConcept = "")
    {
        var pers = GeneratePerson();
    ...
    private static string GenerateCompanyName(string industry)
    {
        if (IndustryTerms.TryGetValue(industry, out var terms))
        {
            var adjective = terms.Adjectives[Random.Next(terms.Adjectives.Count)];
            var element = terms.Elements[Random.Next(terms.Elements.Count)];
            var noun = terms.Nouns[Random.Next(terms.Nouns.Count)];
