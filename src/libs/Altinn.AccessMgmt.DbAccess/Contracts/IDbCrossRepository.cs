...
public interface IDbCrossRepository<T, TExtended, TA, TB> : IDbExtendedRepository<T, TExtended>
{
    ...
    /// </returns>
    Task<IEnumerable<TA>> GetA(Guid id, RequestOptions options, List<GenericFilter> filters, CancellationToken cancellationToken);
    ...
    /// </returns>
    Task<IEnumerable<TB>> GetB(Guid id, RequestOptions options, List<GenericFilter> filters, CancellationToken cancellationToken);
}
