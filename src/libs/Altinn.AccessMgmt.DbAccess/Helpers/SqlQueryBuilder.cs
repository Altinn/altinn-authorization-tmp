...
public class SqlQueryBuilder(DbDefinition definition)
{
    ...
    /// </summary>
    public string BuildBasicSelectQuery(RequestOptions options, IEnumerable<GenericFilter> filters, CrossRelationDefinition? crossDef = null)
    {
    ...
    /// </summary>
    public string BuildExtendedSelectQuery(RequestOptions options, IEnumerable<GenericFilter> filters, CrossRelationDefinition? crossDef = null)
    {
        var sb = new StringBuilder();
    ...
    private string GenerateCrossReferenceJoin(CrossRelationDefinition crossRef, RequestOptions options)
    {
        bool useHistory = options.AsOf.HasValue;
        var crossRefDef = DefinitionStore.TryGetDefinition(crossRef.CrossType) ?? throw new InvalidOperationException();
    ...
    /*Extended*/
    private string GetJoinPostgresStatement(RelationDefinition join, RequestOptions options)
    {
    ...
    private string GenerateJoinPostgresColumns(RelationDefinition join, RequestOptions options)
    {
    ...
    private string GetJoinPostgresFilterString(RelationDefinition join)
    {
    ...
    private OrderedDictionary<string, string> CreateTable()
    {
        ...
        string primaryKeyDefinition = string.Join(',', pk.Properties.Select(t => $"{t.Key} {Helpers.GetPostgresType(t.Value)} NOT NULL "));
    ...
    private OrderedDictionary<string, string> CreateColumn(PropertyDefinition column)
    {
        var scripts = new OrderedDictionary<string, string>();

        var basic = CreateColumn(GetTableName(includeAlias: false), column.Name, Helpers.GetPostgresType(column.Property).ToString(), column.IsNullable, column.DefaultValue);
        scripts.Add(basic.Key, basic.Query);

        if (_definition.HasTranslation)
        {
            var translation = CreateColumn(GetTableName(includeAlias: false, useTranslation: true), column.Name, Helpers.GetPostgresType(column.Property).ToString(), true, column.DefaultValue);
            scripts.Add(translation.Key, translation.Query);
        }

        if (_definition.HasHistory)
        {
            var history = CreateColumn(GetTableName(includeAlias: false, useHistory: true), column.Name, Helpers.GetPostgresType(column.Property).ToString(), true, column.DefaultValue);
            scripts.Add(history.Key, history.Query);

            if (_definition.HasTranslation)
            {
                var translationHistory = CreateColumn(GetTableName(includeAlias: false, useTranslation: true, useHistory: true), column.Name, Helpers.GetPostgresType(column.Property).ToString(), true, column.DefaultValue);
    ...
    private (string Key, string Query) CreateForeignKeyConstraint(RelationDefinition foreignKey)
    {
